...
if settings.ALLOW_LOCALHOST:
	for provider in settings.PROVIDERS:
		settings.PROVIDERS[provider]['whitelist_ips'].append('127.0.0.1')
...
def send_email(git_pull_params, git_pull_results):
	...
	msg = "A " +git_pull_params['provider'] + " webhook triggered a pull request into the following directory: \n\n" + \
		git_pull_params['repo_dir'] + "\n\n Resulting in the following output: \n\n" + git_pull_results
	subject = subprocess.check_output('hostname', shell=True) + " - git pull - " + git_pull_params['repo_name']
...
def git_pull(git_pull_params):
	...
	if not os.path.exists(dest_directory + '/.git'):
		...
		subprocess.call('git remote add -f origin' + repo_url, cwd=dest_directory, shell=True)

	try:
		subprocess.call('git show-ref --heads ' + branch_name, cwd=dest_directory, shell=True)
	except:
		logger.info("Checking out branch: %s" % branch_name)
		subprocess.call('git checkout -b ' + branch_name + ' origin/' + branch_name, cwd=dest_directory, shell=True)

	logger.debug("Performing git pull on: %s" % branch_name)
	git_pull_results = subprocess.check_output('git pull origin ' + branch_name, cwd=dest_directory, shell=True)
...
def parse_bitbucket_data(data):
	...
	for change in changes:
		if not change['new']['type'] == 'branch' or not change['new']['name']:
